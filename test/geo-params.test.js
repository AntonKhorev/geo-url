...
describe("GeoParams", () => {
	...
	describe("keys", () => {
		...
		test("iterates through parameters", () => {
			const params = new GeoParams("first=one;second=two")
			expect([...params.keys()]).toEqual(["first", "second"])
		})
	})
	...
	describe("forEach", () => {
		...
		test("runs the callback once for one parameter", () => {
			...
			expect(callback).toHaveBeenNthCalledWith(1, "one", "first", params)
			expect(callback.mock.contexts).toEqual([undefined])
		})
		test("runs the callback twice for two parameters", () => {
			...
			expect(callback).toHaveBeenNthCalledWith(1, "one", "first", params)
			expect(callback).toHaveBeenNthCalledWith(2, "two", "second", params)
			expect(callback.mock.contexts).toEqual([undefined, undefined])
		})
		test("runs the callback once for one parameter with a context", () => {
			const params = new GeoParams("first=one")
			const callback = vi.fn()
			params.forEach(callback, "context")
			expect(callback).toHaveBeenCalledTimes(1)
			expect(callback).toHaveBeenNthCalledWith(1, "one", "first", params)
			expect(callback.mock.contexts).toEqual(["context"])
		})
		test("runs the callback twice for two parameters with a context", () => {
			const params = new GeoParams("first=one;second=two")
			const callback = vi.fn()
			params.forEach(callback, "context")
			expect(callback).toHaveBeenCalledTimes(2)
			expect(callback).toHaveBeenNthCalledWith(1, "one", "first", params)
			expect(callback).toHaveBeenNthCalledWith(2, "two", "second", params)
			expect(callback.mock.contexts).toEqual(["context", "context"])
		})
	})
})
